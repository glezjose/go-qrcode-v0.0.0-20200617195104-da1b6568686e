...
// set sets the module at (x, y) to v.
func (m *symbol) setFinders(x int, y int, v bool) {
	m.finderModule[y+m.quietZoneSize][x+m.quietZoneSize] = v
	m.isUsed[y+m.quietZoneSize][x+m.quietZoneSize] = true
}
...
// set2dPattern sets a 2D array of modules, starting at (x, y).
func (m *symbol) set2dFinderPattern(x int, y int, v [][]bool) {
	for j, row := range v {
		for i, value := range row {
			m.setFinders(x+i, y+j, value)
		}
	}
}
...
// bitmap returns the entire symbol, including the quiet zone.
func (m *symbol) bitmapFinders() [][]bool {
	module := make([][]bool, len(m.finderModule))

	for i := range m.finderModule {
		module[i] = m.finderModule[i][:]
	}
